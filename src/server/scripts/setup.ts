...
async function setup() {
  ...
  try {
    ...
    // New call-sheet-2026 bucket (from environment variable)
    const newCallSheetsBucket = env.MINIO_BUCKET_NAME;
    const newCallSheetsExists = await minioClient.bucketExists(newCallSheetsBucket);
    
    if (!newCallSheetsExists) {
      await minioClient.makeBucket(newCallSheetsBucket);
      console.log(`Created bucket: ${newCallSheetsBucket}`);
      
      // Set bucket policy to allow public read access
      const newCallSheetsPolicy = {
        Version: "2012-10-17",
        Statement: [
          {
            Effect: "Allow",
            Principal: { AWS: ["*"] },
            Action: ["s3:GetObject"],
            Resource: [`arn:aws:s3:::${newCallSheetsBucket}/*`],
          },
        ],
      };
      
      await minioClient.setBucketPolicy(newCallSheetsBucket, JSON.stringify(newCallSheetsPolicy));
      console.log(`Set public read policy for bucket: ${newCallSheetsBucket}`);
    } else {
      console.log(`Bucket ${newCallSheetsBucket} already exists`);
    }
  } catch (error) {
